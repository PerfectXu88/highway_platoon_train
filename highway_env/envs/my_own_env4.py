...
class my_own_highway(AbstractEnv):
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            'far_reward': 0.3,
            "equal_speed":0.2,
            "Aktionen_Messung":-0.0,
            "collision_reward": -30,  # The reward received when colliding with a vehicle.
    ...
    def _create_vehicles(self) -> None:
        ...
        #front_vehicle = IDMVehicle.make_on_lane(self.road,
                                                # lane_index=self.config["initial_lane_id"],
                                                 #longitudinal=vehicle.position + 10 * np.float64(Vehicle.LENGTH),
        ...
        for i in range(self.config["other_vehicles"]):
            ...
            vehicle.randomize_behavior()
            self.road.vehicles.append(vehicle)
        for i in range(self.config["still_bleibend"]): #创建障碍车
            vehicle = other_vehicles_type.create_accident_car(cls=Vehicle,road=self.road, spacing=1 / self.config["vehicles_density"])
            self.road.vehicles.append(vehicle)
