...
class MultiObservation(ObservationType):
    def __init__(self,
                 env: 'AbstractEnv',
                 observation_config: dict,
                 **kwargs) -> None:
    ...
    def space(self) -> spaces.Space:
        ...
        for obs_type in self.agents_observation_types:
            tmp.append(obs_type.space())
            num = num + 1
        for i in range(0, num+1):
            long = long + tmp[i].shape[0]
        if len(tmp) == 0:
            return spaces.Tuple(tmp)
        else:
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            wid = tmp[1].shape[1]
            return spaces.Box(shape=(long, wid), low=-np.inf, high=np.inf, dtype=np.float32)

    def observe(self) -> tuple:
        return tuple(obs_type.observe() for obs_type in self.agents_observation_types)


class KiObservation(ObservationType):
    """Observe the kinematics of nearby vehicles."""
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "KiGoal":
        return KiObservation(env, **config)
    ...
    elif config["type"] == "MultiObservation":
        return MultiObservation(env, **config)
