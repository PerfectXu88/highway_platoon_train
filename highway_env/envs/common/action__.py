...
class ContinuousAction(ActionType): # 连续型
...
class DiscreteAction(ContinuousAction): # 标准离散行为
    def __init__(self,
                 env: 'AbstractEnv',
                 acceleration_range: Optional[Tuple[float, float]] = None,
                 steering_range: Optional[Tuple[float, float]] = None,
...
class DiscreteMetaAction(ActionType): # 离散的
...
class MultiDiscreteMetaAction(ActionType):
    def __init__(self, env: 'AbstractEnv',
        ...
        self.NUM_CARS = 3
        # self.target_speeds = np.array(target_speeds) if target_speeds is not None else MDPVehicle.DEFAULT_TARGET_SPEEDS
        self.CAR1 = self.CAR2 = self.CAR3 = ['LANE_LEFT','IDLE','LANE_RIGHT','FASTER','SLOWER']
        self.actions = {
            (5 ** 2)*i + 5*j + k: (self.CAR1[i],self.CAR2[j],self.CAR3[k]) \
        for i in range(5) for j in range(5) for k in range(5)
        }
        self.actions_indexes = {v: k for k, v in self.actions.items()}
        self.actions[125] = ('MAINTAIN', 'MAINTAIN', 'MAINTAIN')
    ...
    def act(self, action: Union[int, np.ndarray]) -> None:
        ...
        for controlled_vehicle in self.env.controlled_vehicles:
            controlled_vehicle.act(self.actions[int(action)][index])
            index += 1
