...
class jam_env(AbstractEnv):
        ...
        if self.config["normalize_reward"]:
            reward = utils.lmap(reward,
                                [self.config["collision_reward"],
                                 ...
                                 self.config["var_speed"]+
                                 self.config["front_car"]
                                 ],
                                [0, 1])
    ...
    def _rewards(self, action: Action) -> Dict[Text, float]:
        ...
        num_frount_back_car=0
        for this_vehicle in self.road.vehicles:
            if this_vehicle.position[0]>max(distance):
                num_frount_back_car=num_frount_back_car+1
            if this_vehicle.position[0]>min(distance):
                num_frount_back_car=num_frount_back_car+1
        ...
        return {
            ...
            "equal_speed" : float(1/(max(equal_speed)-min(equal_speed)+1)),
            "var_speed": float(1 / (np.var(equal_speed) + 1)),
            ...
            "front_car":float(1/(num_frount_back_car-20))
        }
