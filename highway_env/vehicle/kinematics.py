...
class Vehicle(RoadObject):
    ...
    def create_accident_car(cls, road: Road,
                      speed: float=0, #写死就行，其他参数照random
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
        ...
        _id = int(0)
        lane = road.network.get_lane((_from, _to, _id))
        # position=road.np_random.uniform(Vehicle.DEFAULT_INITIAL_SPEEDS[0], Vehicle.DEFAULT_INITIAL_SPEEDS[1])
        # default_spacing = 12 + 1.0 * position  #随机化位置，免得挤成一坨
        # offset = spacing * default_spacing * np.exp(-5 / 40 * len(road.network.graph[_from][_to]))
        # x0 = np.max([lane.local_coordinates(v.position)[0] for v in road.vehicles]) \
        #     if len(road.vehicles) else 3 * offset
        # x0 += offset * road.np_random.uniform(0.9, 1.1)
        ...
        return cls(road, lane.position(longitudinal, lateral), lane.heading_at(longitudinal), speed)
        # return v



    def create_accident_slow_car(cls, road: Road,
                      speed: float=0, #写死就行，其他参数照random
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
    ...
    def create_accident_fast_car(cls, road: Road,
                      speed: float=25, #写死就行，其他参数照random
                      lane_from: Optional[str] = None,
                      lane_to: Optional[str] = None,
                      lane_id: Optional[int] = None,
                      longitudinal:Optional[float]= None,
