...
if __name__ == "__main__":
    ...
    if train:
        ...
        # Train the agent
        model.learn(total_timesteps=10*10000)
        # Save the agent
        model.save("highway_attention_ppo/model")

    model = PPO.load("highway_attention_ppo/model_jam_attention")
    env = make_configure_env(**env_kwargs)
    # ————————————————————————————————
    env = gym.make('jam_env', render_mode='rgb_array')
    env = RecordVideo(env, video_folder="highway_attention_ppo/video_0", episode_trigger=lambda e: True)
